...
#'  count). The kernel weights a feature's own marginal statistic equally
#'  against its neighbors' (fixed 0.5 self-contribution, not user-adjustable,
#'  to prevent tuning this parameter toward a desired result). Statistical
#'  significance is assessed by comparing the smoothed statistic against a
#'  null distribution generated through permutation.
#' }
#'
...
#'
#' @usage analyzeLipidRegion(lipid_se, ref_group, split_chain=FALSE,
#' chain_col=NULL, radius=3, test="t.test", abund_weight=TRUE,
#' permute_time=100000)
...
analyzeLipidRegion <- function(
        ...
        test="t.test", abund_weight=TRUE, permute_time=100000){
    ...
    X <- SummarizedExperiment::assay(lipid_se) %>% t()
    if (ncol(X) < 2) {
        stop("Lipid characteristic (e.g., chain length, double bond) must ",
            "contain at least two features.")
    }
    X.info <- SummarizedExperiment::rowData(lipid_se) %>% as.data.frame()
    group_info <- SummarizedExperiment::colData(lipid_se) %>% as.data.frame()
    if (length(unique(group_info$group)) != 2) {
        stop(
            "analyzeLipidRegion() requires exactly 2 groups; found ",
            length(unique(group_info$group)))
    }
    ...
    if (isTRUE(split_chain)) {
        if (!is.null(chain_col) && chain_col %in% colnames(X.info)) {
            output.df <- .analyzeSplitChain(
                X, X.info, group, chain_col, radius, test,
        ...
        output.df <- .stat_lipidTrend(
            X, X.info, group, radius, test, permute_time, abund_weight)
        return(new(
...
.analyzeSplitChain <- function(
    ...
    output.df <- lapply(names(chain_groups), function(chain_type) {
        ...
        .stat_lipidTrend(
            ...
            group, radius, test, permute_time, abund_weight)
    })
...
.regionStat <- function(X, Y, test="t.test", ...){
    ...
    if (test == "t.test") {
        ...
        df <- n1 + n2 - 2
        t.test.pval <- 2 * stats::pt(abs(t.stat), df=df, lower.tail=FALSE)
...
.stat_lipidTrend <- function(
    ...
    stat_res <- .smooth_permutation(
        X, group, dist.mat, test, abund_weight, permute_time,
        region.stat.obs)
    ...
    if(all(is.na(output.df$smoothing.pval.BH))){
        stop(
            "Gaussian kernel smoothing may fail due to some lipids having zero",
            " variance in abundance between the two groups or having identical",
            " sample values. Please remove those lipids and re-run the ",
            "function.")
    }
    return(output.df)
}
...
.smooth_permutation <- function(
    ...
    # Self-contribution is fixed at 0.5 (equal weight between a feature's
    # own signal and its neighbors'), not user-adjustable, so this value
    # cannot be tuned toward a desired result.
    own_contri <- 0.5
    ...
    if (radius.idx + 1 > length(radius.seq)) {
        stop(
            "Based on the data structure, the fixed self-contribution ",
            "(own_contri=0.5) cannot be achieved with the current ",
            "'radius' setting. Try a different 'radius'.")
    }
...
.resultSummary <- function(
    ...
    smooth.stat.pval.BH <- stats::p.adjust(smooth.stat.pval, method="BH")
    direction <- ifelse(sign(smooth.stat) > 0, "+", "-")
    marginal.pval <- 10^-abs(region.stat.obs)
    marginal.pval.BH <- stats::p.adjust(marginal.pval, method="BH")
    log2.FC <- apply(
    ...
    if (dimension == 2) {
        result %<>% dplyr::mutate(avg.abund=colMeans(X), .before="direction")
    }
    if (dimension == 1) {
        result %<>% dplyr::mutate(
            avg.abund.ctrl=colMeans(X[group == 0,]),
            avg.abund.case=colMeans(X[group == 1,]), .before="direction")
    }
    return(result)
}
