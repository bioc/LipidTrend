...
one-dimensional and two-dimensional analyses of lipid features (e.g.,
...
(e.g., double bond count, chain length, or other numeric variables),
...
lipid feature, such as chain length or double bond count.
...
smoothed testing to identify meaningful trends across lipid features
...
res1D <- analyzeLipidRegion(
    ...
    chain_col="chain", radius=3, test="t.test",
...
**Note:**
- These ribbons are shaded based on an independent region-level paired
t-test (see the [Regional Test and Fold-Change section](#regionalTestFC)).
The test needs more than 5 significant features in a direction to be
reliable; when either direction has 5 or fewer, the whole figure falls
back to shading by the smoothed permutation test alone.


## Two-dimensional analysis {#lipid2d}

Two-dimensional analysis is applicable when the input dataset includes two
lipid features, such as chain length, double bond count, or other
...
res2D <- analyzeLipidRegion(
    ...
    chain_col="Total.C", radius=3, test="t.test",
    abund_weight=TRUE, permute_time=1000)
...
The X- and Y-axes represent two lipid characteristics (e.g., total
...
**Note:**
- These outlines are shaded based on an independent region-level paired
t-test (see the [Regional Test and Fold-Change section](#regionalTestFC)).
The test needs more than 5 significant features in a direction to be
reliable; when either direction has 5 or fewer, the whole figure falls
back to shading by the smoothed permutation test alone.
...
## Regional Test and Fold-Change {#regionalTestFC}
While `smoothing.pval.BH` already provides a permutation-based significance
call for each region, an independent, more directly interpretable statistic
is often useful for reporting or downstream review. `regionalTestFC()`
supplements the `LipidTrendSE` result with a region-level paired t-test and
fold-change summary for each trend direction (Increase / Decrease).

For each direction, the significant features identified via
`smoothing.pval.BH < p_cutoff` are treated as paired observations (feature
= pairing unit), and a paired t-test is performed on their group-mean
abundances.

```{r regionalTestFC_1D}
# region-level paired t-test and fold-change, even-chain result
regionalTestFC(res1D)$even_result
```

The returned table has one row per direction:

* **direction**: `"Increase"` or `"Decrease"`.
* **n.features**: Number of significant features in that direction.
* **regional.test.pval**: Paired t-test p-value on feature-wise group-mean
abundance across the significant features. `NA` when `n.features <= 5` (a
paired t-test on 5 or fewer pairs is too underpowered to be trustworthy);
each direction is evaluated independently of the other.
* **regional.FC** / **log2.regional.FC**: The regional fold-change, on the
linear and log2 scale respectively (`regional.FC` = `2^log2.regional.FC`).
This is the **median** fold-change across the significant features in that
direction -- a median rather than a mean, so that a single extreme feature
cannot dominate the summary of the region.

**Note:**
- If `split_chain=TRUE` in `analyzeLipidRegion()`, `regionalTestFC()`
returns a list with `even_result`/`odd_result`, matching
`even_chain_result()`/`odd_chain_result()`. Otherwise it returns a single
data frame.<br>
- `p_cutoff` here defines region membership (same role as elsewhere in the
package); it does *not* threshold `regional.test.pval` itself, so the raw
p-value is always reported for the caller to interpret.

### Region validation in `plotRegion1D()` / `plotRegion2D()`
`plotRegion1D()` and `plotRegion2D()` consult `regionalTestFC()`
internally, and shade a direction only after its region-level paired
t-test passes (`regional.test.pval < regional_p_cutoff`); an unvalidated
direction is left unshaded. The paired t-test is only reliable with more
than 5 significant features in a direction, so when either direction in a
figure has 5 or fewer, there are too few features to run the test, and the
whole figure falls back to shading by the smoothed permutation test alone
(`smoothing.pval.BH < p_cutoff`) instead, so a single figure never mixes
two different statistical criteria.

**Note:**
- `regional_p_cutoff` (default `0.05`) controls this validation step and is
independent from `p_cutoff`, which only governs region membership.<br>
- Because this depends on the data, re-running `plotRegion1D()`/
`plotRegion2D()` on an existing `LipidTrendSE` object after upgrading may
change the appearance of figures where a direction has more than 5
significant features, even without any code changes.
